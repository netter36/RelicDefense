...
3. **다양한 공격 타입**: 7가지 고유한 공격 방식 (Normal, Rapid, Laser, Nova, Bomb, Multi, Chain) + 특수 비주얼 이펙트 (Arrow Rain, Spear Strike)
4. **8가지 속성**: Fire, Ice, Thunder, Leaf, Gem, Shadow, Plasma, Mystic
5. **무한 난이도**: 시간이 지날수록 강해지는 적들
6. **실시간 전투**: 60 FPS 부드러운 액션
7. **성능 모니터링**: 실시간 FPS 카운터 지원
...
- **메탈릭 UI 디자인**: 미래지향적이고 깔끔한 모듈 블록 디자인
...
### 🎨 시각적 완성도
- **Phaser.Geom 기반 렌더링**: 이미지 에셋 없이 코드로 생성되는 고품질 절차적 그래픽
- **화려한 이펙트**:
  - **Arrow Rain (화살비)**: 하늘에서 쏟아지는 화살 공격 (Leaf 속성)
  - **Spear Strike (창 격돌)**: 대각선으로 꽂히는 강력한 창 공격 (Thunder/Judgement)
  - **Particle Effects**: 타격감 있는 피격 및 폭발 이펙트
- **개선된 UI**: 그라데이션과 애니메이션이 적용된 콤보 리스트 및 다크 모드 테마
...
| **Spear** | 대각선 타격 | 하늘에서 대각선으로 꽂히는 창 (Thunder/Judgement) | 특수 효과 |
| **Arrow Rain** | 화살비 | 하늘에서 수직으로 떨어지는 화살 (Leaf) | 특수 효과 |
...
| 유물 | 형태 | 공격 타입 | 공격력 | 사거리 | 공속 | 특수 능력 |
|:---|:---:|:---:|:---:|:---:|:---:|:---|
| **선령의 활** | 1×3 | Normal | 20 | 400 | 800ms | 장거리 저격 (화살비 이펙트) |
...
| **심판의 프리즘** | 2×2 | Chain | 18 | 350 | 1200ms | 3회 연쇄 (창 격돌 이펙트) |
...
- **공격 타입 구현**:
  ...
  - `fireBomb()` - 폭발 광역
  - `fireChain()` - 연쇄 타격
- **비주얼 이펙트 연동**: Arrow Rain, Spear Strike 등 `RenderUtils` 호출
- **디버프 적용**: 효율 계산 및 지속시간 관리
- **치명타 계산**: 확률 기반 데미지 증폭

##### EnemySystem.js
- **몬스터 풀링**: 성능 최적화
- **Dirty Flag 패턴**: `isStatusDirty`를 사용하여 변경 시에만 HP바/상태아이콘 다시 그리기
- **경로 탐색**: 웨이포인트 기반 이동
- **체력 바**: 실시간 HP 표시
- **스폰 로직**: 난이도 기반 조절
...
#### 렌더링 최적화
- **Dirty Flag 패턴**: 적의 HP바나 상태 아이콘은 실제 변경(피격, 디버프)이 있을 때만 다시 그리기 (`isStatusDirty`)
