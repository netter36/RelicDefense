...
import { SLOT_SIZE, ELEMENT_COLORS } from '../constants';
...
export class RenderUtils {
     ...
     */
    static renderProceduralShape(scene: Phaser.Scene, container: Phaser.GameObjects.Container, data: Tower) {
        ...
        const color = data.element ? (ELEMENT_COLORS[data.element] || 0x64748b) : 0x64748b;
        const darkColor = this.getDarkColor(data.element || '');
        
        const check = (x: number, y: number) => data.shape && data.shape[y] && data.shape[y][x];

        // 1. Base & Frame Rendering
        if (data.shape) {
            data.shape.forEach((row: number[], dy: number) => {
        ...
        // Synergy Glow (Outer Aura) - 에너지 효과 제거
        // if (data.isSynergetic || data.activeSynergy) {
        //     // ... 효과 제거 ...
        // }
    }

    static getDarkColor(element: string) {
    ...
    static renderUniqueDetail(g: Phaser.GameObjects.Graphics, data: Tower) {
        ...
        const drawSymbol = (type: string, x: number, y: number, scale = 1) => {
        ...
        if (data.type === 'tablet') {
            ...
            const buffType = (data as any).buff ? (data as any).buff.type : 'atk';
        ...
        switch (id) {
            ...
            case 'mystic_scroll': // Holo-Scroll
                ...
                g.fillPoints([
                    new Phaser.Math.Vector2(-10, -25),
                    new Phaser.Math.Vector2(10, -25),
                    new Phaser.Math.Vector2(15, -5),
                    new Phaser.Math.Vector2(-15, -5)
                ], true);
                break;

            default:
                if (data.element) drawSymbol(data.element, 0, 0, 1.2);
        }
    }

    static renderMonsterByType(g: Phaser.GameObjects.Graphics, type: any) {
    ...
    static getHitEffectTexture(scene: Phaser.Scene, color: number) {
    ...
    static showHitEffect(scene: any, x: number, y: number, color: number | string) {
        ...
        const textureKey = this.getHitEffectTexture(scene, finalColor as number);
    ...
    static getProjectileColor(item: any) {
    ...
    static getProjectileTexture(scene: Phaser.Scene, color: number) {
    ...
    static createProjectile(scene: any, x: number, y: number, target: any, item: any) {
    ...
    static destroyProjectile(proj: Phaser.GameObjects.Image) {
        if (proj) {
    ...
    static getArrowTexture(scene: Phaser.Scene, color: number) {
    ...
    static createArrowRain(scene: any, x: number, y: number, color: number = 0x4ade80, onImpact: () => void) {
        const count = 5;
    ...
    static createSpearStrike(scene: any, x: number, y: number, color: number = 0xffff00) {
    ...
    static getMineTexture(scene: Phaser.Scene, color: number) {
    ...
    static createMine(scene: any, x: number, y: number, color: number) {
    ...
    static showFloatingText(scene: any, x: number, y: number, text: string, color = '#ffffff') {
