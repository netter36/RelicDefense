...
import { GRID_WIDTH, GRID_HEIGHT, SLOT_SIZE, CANVAS_WIDTH, CANVAS_HEIGHT, GAME_CONFIG } from '../constants';
...
export class GridSystem {
    ...
    grid: (Tower | null)[][];
    placedItems: Tower[];
    gridStartPos: { x: number, y: number };
    hoveredItem: Tower | null = null;
    hoverRange: Phaser.GameObjects.Graphics | null = null;
    ...
    createItem(id: string, gx: number, gy: number, isRotated = false): Tower | null {
        const template = ITEMS.find(i => i.id === id);
        if (!template) return null;

        const item = JSON.parse(JSON.stringify(template)) as Tower;
        ...
        if (item.width === undefined && item.shape) item.width = item.shape[0].length;
        if (item.height === undefined && item.shape) item.height = item.shape.length;

        // [추가] 생성 시 회전 적용
        if (isRotated && item.shape) {
            // Rotate Shape
        ...
        container.setInteractive({
            ...
            hitAreaCallback: (hitArea: any, x: number, y: number, gameObject: any) => {
                const col = Math.floor(x / SLOT_SIZE);
                const row = Math.floor(y / SLOT_SIZE);
                return item.shape && item.shape[row] && item.shape[row][col];
            }
        });
        ...
        container.on('pointerover', (pointer: Phaser.Input.Pointer) => {
            ...
            // Cast pointer to any because showTooltip expects MouseEvent-like but Phaser Pointer works
            this.uiManager.showTooltip(pointer as any, item, true);
            // Show Range on Hover
            if (item.type !== 'tablet') {
                if (!this.hoverRange) {
    ...
    autoPlaceItem(id: string): boolean {
        const template = ITEMS.find(i => i.id === id);
        if (!template) return false;

        const w = template.width || (template.shape ? template.shape[0].length : 1);
        const h = template.height || (template.shape ? template.shape.length : 1);
        const dummy: any = { shape: template.shape, width: w, height: h };
    ...
    removeItem(item: Tower) {
        // [추가] 제거 중인 아이템이 호버 상태라면 정리 (툴팁 및 사거리 표시 제거)
        ...
        // [추가] 타워 시스템 정리 (좀비 투사체 방지)
        const towerSystem = (this.scene as any).towerSystem;
        if (towerSystem && item.type === 'artifact') {
            towerSystem.cleanupTower(item);
        }
        ...
        if ((this.scene as any).addGold) (this.scene as any).addGold(Math.floor(cost * 0.5));
    ...
    setGrid(item: Tower, val: Tower | null) {
        if (!item.gridPos || !item.shape) return;
    ...
    canPlace(item: any, x: number, y: number) {
        if (x < 0 || y < 0 || x + item.width > GRID_WIDTH || y + item.height > GRID_HEIGHT) return false;
        return item.shape.every((row: any[], dy: number) => row.every((v, dx) => !v || !this.grid[y + dy][x + dx]));
    }

    rotateItem(item: Tower) {
        ...
        if (this.canPlace(item, placeX, placeY)) {
            ...
            this.snap(item);
            this.setGrid(item, item);
            if (item.el) RenderUtils.renderProceduralShape(this.scene, item.el, item);
            ...
            if (item.el) {
                ...
                this.scene.input.setDraggable(item.el); // 드래그 가능 상태 복구
            }

            this.calculateSynergies();

            // Re-show range if hovering
            if (this.hoverRange && item.el) {
                this.hoverRange.clear();
                this.hoverRange.lineStyle(2, 0xffffff, 0.4);
                this.hoverRange.fillStyle(0xffffff, 0.05);
    ...
    calculateSynergies() {
        ...
        this.scene.events.emit('hud-update', { combos: activeCombos, dps: Math.floor(totalDPS) });
    }
}
